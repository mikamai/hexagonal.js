...
class FlatToppedMap extends AbstractMap
  ...
  _sharedVerticesFromNeighborsOfOffset: (row, col, vertices) =>
    ...
    @_sharedVerticesFromNeighbor vertices, @at(row + 1, col), 5: 1, 4: 2
    if col % 2 is 0
      ...
      @_sharedVerticesFromNeighbor vertices, @at(row + 1, col - 1), 0: 2, 5: 3
    else
      @_sharedVerticesFromNeighbor vertices, @at(row, col - 1), 1: 3, 0: 4
      @_sharedVerticesFromNeighbor vertices, @at(row, col + 1), 3: 1, 4: 0
  ...
  _sharedEdgesFromNeighborsInOffset: (row, col, halfEdges) =>
    ...
    if col % 2 is 0
      ...
      @_sharedEdgesFromNeighbor(halfEdges, @at(row + 1, col - 1), 5: 2)
    else
      @_sharedEdgesFromNeighbor(halfEdges, @at(row, col - 1), 0: 3)
      @_sharedEdgesFromNeighbor(halfEdges, @at(row, col + 1), 3: 0)

  _newHexagon: (halfEdges) =>
    new Hexagon halfEdges, precision: @precision, flatTop: true
